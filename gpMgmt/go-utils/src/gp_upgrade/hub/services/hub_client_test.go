...
var _ = Describe("HubClient", func() {
	...
	It("closes open connections when shutting down", func() {
		...
		hub, shutdown := services.NewHubClient(nil, reader, grpc.DialContext)
	...
	It("retrieves the agent connections from the config file reader", func() {
		...
		hub, shutdownHub := services.NewHubClient(nil, reader, grpc.DialContext)
		defer shutdownHub()
	...
	It("returns an error if the config reader fails", func() {
		...
		hub, shutdownHub := services.NewHubClient(nil, reader, nil)
		defer shutdownHub()
