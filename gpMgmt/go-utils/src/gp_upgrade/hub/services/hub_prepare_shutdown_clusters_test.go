	...
	Describe("PrepareShutdownClusters", func() {
		Describe("ignoring the go routine", func() {
			initialSetup := func() (pb.CliToHubServer, *pb.PrepareShutdownClustersRequest) {
				...
				listener, shutdownHub := services.NewHubClient(&fakeStubClusterPair{}, &reader, grpc.DialContext)
				defer shutdownHub()
			...
			It("fails if the cluster configuration setup can't be loaded", func() {
				...
				reader := configutils.NewReader()
				failingListener, shutdownHub := services.NewHubClient(&fakeFailingClusterPair{}, &reader, grpc.DialContext)
				defer shutdownHub()
